...
pynbody.analysis.angmom.faceon(h6)
...
c = np.savetxt('sonia_4.6_data.data', np.column_stack((x2,y2)), delimiter=',')
...
#save x and y into a data file
c = np.savetxt('sonia_4.6_data_filt.data', np.column_stack((x,y)), delimiter=',')


#calculate distance for separation
#finds number of BH
def findBH(s):
     BHfilter = np.where((s.stars['iord']==75288565)|(s.stars['iord']==75288831))
     BH = s.stars[BHfilter]
     return BH
BH = findBH(s)
...
with pynbody.analysis.halo.center(h[6], mode='hyb'):
    print (h[6]['pos'][0])
    print (h[6]['pos'][1])
    print (h[6]['pos'][2])
print('pos')
...
starmass = h[6].s['mass'].sum()
gasmass = h[6].g['mass'].sum()
virialmass = starmass+gasmass+h[6].d['mass'].sum()
data = ['halo: ',6,'BH_ID: ', BH['iord'],'time: ', gettime(s),'z: ',getz(s),'BH_mass: ', BH['mass'],'Radius: ', BH['r'],'star_mass: ', starmass,'gas_mass: ', gasmass,'virial_mass: ', virialmass]
