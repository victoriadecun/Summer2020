...
for M, z, q, letter in [[5.64555e6, 1.9163191, 0.0089485200, 'A'], [7.85521e6, 0.39280810, 0.25782200, 'B'], [1.48301e6, 10.762340, 0.19103100, 'C'], [4.66498e6, 9.4247567, 0.29180500, 'D'], [82245.4, 3.7944614, 0.47851300, 'E'], [8.29909e6, 1.7536451, 0.67958900, 'F'], [676377, 11.378336, 0.86115400, 'G'], [2.58093e6, 9.6918367, 0.44843200, 'H'], [2.24031e6, 4.6032768, 0.30951800, 'I'], [1.70916e6, 8.9868174, 0.34992000, 'J'], [336181.25, 5.2838543, 0.025792000, 'K']]:
    ...
    #plots an assigns colors to each redshift
    if letter is 'A':
        ax.loglog(f[ins], hc[ins], f[mrg], hc[mrg], f[rd], hc[rd], color='black', lw=2)
    if letter is 'C':
        ax.loglog(f[ins], hc[ins], f[mrg], hc[mrg], f[rd], hc[rd], color='black', lw=2)
    if letter is 'G':
        ax.loglog(f[ins], hc[ins], f[mrg], hc[mrg], f[rd], hc[rd], color='black', lw=2)
    ...
    if letter is 'B':
        ax.loglog(f[ins], hc[ins], f[mrg], hc[mrg], f[rd], hc[rd], color='darkviolet', lw=2)
    if letter is 'E':
        ax.loglog(f[ins], hc[ins], f[mrg], hc[mrg], f[rd], hc[rd], color='darkviolet', lw=2)
    if letter is 'F':
        ax.loglog(f[ins], hc[ins], f[mrg], hc[mrg], f[rd], hc[rd], color='darkviolet', lw=2)
    ...
    if letter is 'D':
        ax.loglog(f[ins], hc[ins], f[mrg], hc[mrg], f[rd], hc[rd], color='magenta', lw=2)
    
    #ax.loglog(f[ins], hc[ins],  lw=2)
    #ax.loglog(f[mrg], hc[mrg],  lw=2)
    #ax.loglog(f[rd], hc[rd],  lw=2)

    #the axs do the colors for the ins,mrg,rd
...
ax.plot(1,1, color='darkviolet', ls='solid', label='Will Merge')
ax.plot(1,1, color='magenta', ls='solid', label="Won't Merge")
ax.plot(1,1, color='black', ls='solid', label='Unresolved')
...
fig.savefig('gwstrainplot_NEW.pdf', dpi=200)
plt.show()
